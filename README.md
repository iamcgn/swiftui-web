...
| `Packages/UIKitWeb` | `import UIKit` on the same substrate: views, layers, controls, view controllers, Auto Layout (a Cassowary solver), `draw(_:)` with UIBezierPath, `UIView.animate`, navigation and tab bar controllers, `UITableView`, sliders, segmented controls, steppers, progress and activity indicators, page controls; `Examples/UIKitCounter`; goldens from real UIKit on an iPhone simulator (`scripts/gen-goldens-sim.sh uikit`, `Fixtures/UIKit`) |
