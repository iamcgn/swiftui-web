...
| `Packages/UIKitWeb` | `import UIKit` on the same substrate: views, layers, controls, view controllers, Auto Layout (a Cassowary solver), `draw(_:)` with UIBezierPath; `Examples/UIKitCounter`; goldens from real UIKit on an iPhone simulator (`scripts/gen-goldens-sim.sh uikit`, `Fixtures/UIKit`) |
| `Sources/SwiftUIWebUIKit` | `UIViewRepresentable` and `UIViewControllerRepresentable` over UIKitWeb; `import SwiftUI` re-exports `UIKit` as on iOS (`Docs/elements/Representable.md`) |
