...
# Usage: scripts/tier-b.sh [--no-build] [--filter layout/] [--port 8766]
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BUILD=1; FILTER=""; PORT=8766
while [[ $# -gt 0 ]]; do
  case "$1" in
...
sleep 1
cd "$ROOT/Playwright" && node tier-b.mjs "http://127.0.0.1:$PORT/index.html" --filter "$FILTER" --out "$ROOT/.build/tier-b"
