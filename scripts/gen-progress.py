...
def render_landing(support):
    """The landing page's summary: the counts per framework and status (the full matrix lives on
    the progress page since 2026-09-18, which keeps the landing bundle light)."""
    counts = Counter(e["status"] for s in support["sections"] for e in s["entries"])
    count_literal = ", ".join(f".{status}: {counts[status]}" for status in SUPPORT_STATUSES if counts[status])
    frameworks = []
    for fw in SUPPORT_FRAMEWORKS:
        c = Counter(e["status"] for s in support["sections"] if s["framework"] == fw for e in s["entries"])
        literal = ", ".join(f".{status}: {c[status]}" for status in SUPPORT_STATUSES if c[status])
        frameworks.append(f'        SupportFramework(name: "{fw}", counts: [{literal}]),')
    return f'''// Generated by scripts/gen-progress.py from Docs/support.json (last edited {support['generated']}); do not edit.
...
enum SupportData {{
    static let generated = "{support['generated']}"
