...
| `Canvas / GraphicsContext (fill, stroke, text, transforms, opacity, clip, drawLayer) / ResolvedText` | 🟢 partial | Immediate-mode drawing recorded into the display list (a concat op for rotated text); pixels within 0.11 % of Apple's; no gradients, images, blend modes, filters or symbols | canvas/basic, canvas/sizing |
| `Gradient / LinearGradient / RadialGradient / AngularGradient (fills, strokes, strokeBorder, backgrounds)` | 🟢 partial | Display-list gradient ops painted by Canvas2D, stops blended in Oklab to match Apple's perceptual blending (≤ 0.08 % pixels); no text/image foreground gradients, elliptical or mesh gradients | gradient/basic |
...
| `Image(systemName:) / Image.Scale / imageScale(_:)` | 🟠 stub | Compiles; a symbol image lays out at 0 × 0 and draws nothing until the icon table lands |  |
