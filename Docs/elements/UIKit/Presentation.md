...
- `UIAlertController`: `init(title:message:preferredStyle:)` (`alert`, `actionSheet`),
  `message`, `actions`, `addAction`, `preferredAction`, `addTextField` (stored, not laid out
  yet), presented with `present(_:animated:completion:)`; tapping an action dismisses the alert
  and runs its handler. `UIAlertAction`: `init(title:style:handler:)` (`default`, `cancel`,
